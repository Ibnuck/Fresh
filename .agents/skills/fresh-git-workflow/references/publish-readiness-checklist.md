# Fresh Publication Readiness

```markdown
## Identity
- [ ] Working directory is `/Users/ibnutaufickahraza/Swift/Fresh`.
- [ ] Git root exactly matches the working directory.
- [ ] Remote owner/name have been verified and do not target another repo.

## Goal gate
- [ ] Goal ID and outcome summary exist.
- [ ] Acceptance criteria are complete.
- [ ] Intended diff and untracked files were inspected.
- [ ] No unrelated changes or secrets are staged.
- [ ] Build/tests/manual checks pass against the current tree.
- [ ] A new context-clean reviewer reviewed the current result.
- [ ] Final verdict is `No material findings`.

## Publication
- [ ] Branch follows the Fresh roadmap convention.
- [ ] Commit subject describes outcome and follows the type convention.
- [ ] Cached diff was inspected before commit.
- [ ] Push target matches the verified origin.
- [ ] PR target is `dev` for a goal or `main` for an approved milestone.
- [ ] PR body records verification, review cycles, risks, and exclusions.
```
